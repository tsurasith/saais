...
?>
  ...
  <? 
			...
			if($grade_source == 1)
			{
				$sqlRoom = "select distinct registeryear as display, right(registeryear,4) as acadyear,left(registeryear,1) as acadsemester
							from tb_register 
							where concat('0',studentid) = '" . $_SESSION['username'] . "' order by right(registeryear,4),left(registeryear,1)";
			}else
			{
				$sqlRoom = "select 
									distinct concat(acadsemester,'/',acadyear) as display 
							from grades 
							where student_id = '" . $_SESSION['username'] . "' order by acadyear,acadsemester";
			}
			
			$_resSemester = mysql_query($sqlRoom); 
	?>
      ...
      <table class="admintable"  cellpadding="1" cellspacing="1" border="0" align="center">
        ...
        <? 	$_sql =   "select 
			  ...
			  $_sql2 =   "select 
						...
						where 
								a.studentid = right('" . $_SESSION['username'] . "',4)
								and left(registeryear,1) = '" . $_Term['acadsemester'] . "'
								and right(registeryear,4) = '" . $_Term['acadyear'] . "'
						order by 1,2 desc";	
